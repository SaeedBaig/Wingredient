...
def upload_recipe(username, name, cooking_time, difficulty, serving_size, cooking_notes, description, imageRef, method, ingredients, ingredient_quantities, equipment):
    ingredient_ids = get_ingredient_ids_from_names(ingredients)
    equipment_ids = get_equipment_ids_from_names(equipment)
    with db.getconn() as conn:
        with conn.cursor() as cursor:
            recipe_id = gen_id()
            query = "INSERT INTO Recipe (id, name, time, difficulty, serving, notes, description, imageRef, method) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)" 
            cursor.execute(query, (recipe_id, name, cooking_time, difficulty, serving_size, cooking_notes, description, imageRef, method))
            conn.commit()
